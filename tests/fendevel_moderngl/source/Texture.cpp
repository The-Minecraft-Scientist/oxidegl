...
GLuint create_texture_2d(GLenum internal_format, GLenum format, GLsizei width,
                         GLsizei height, void *data, GLenum filter,
                         GLenum repeat, bool generateMipMaps) {
  GLuint tex = 0;
  ...
  if (data) {
    glTextureSubImage2D(tex, 0, 0, 0, width, height, format, GL_UNSIGNED_BYTE,
                        data);
  }
...
GLuint create_texture_2d_from_file(std::string_view filepath, stb_comp_t comp,
  ...
  const auto name = create_texture_2d(in, ex, x, y, data, GL_LINEAR, GL_REPEAT,
                                      generateMipMaps);
...
GLuint
create_texture_cube_from_file(std::array<std::string_view, 6> const &filepath,
                              stb_comp_t comp) {
  int x, y, c;
