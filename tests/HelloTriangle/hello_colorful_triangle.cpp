...
int main() {
  ...
  // --------------------
  GLFWwindow *window =
      glfwCreateWindow(800, 600, "Hello Colorful Triangle", NULL, NULL);
  if (window == NULL) {
    const char *desc;
    glfwGetError(&desc);
  ...
  // glBindBuffer(GL_ARRAY_BUFFER, VBO); // Binding to openGl context was
  // necessary
  //   replaced with:
  ...
  //   replaced with:
  glNamedBufferStorage(hctVBO, sizeof(vertices), vertices,
  ...
  GLuint vaoBindingPoint =
      0; // A binding point in VAO. See GL_MAX_VERTEX_ATTRIB_BINDINGS
  ...
  //   replaced with:
  glEnableVertexArrayAttrib(hctVAO,
                            attribPos); // Need to precise vao, as there is no
                                        // context binding in DSA style
  glEnableVertexArrayAttrib(
      hctVAO,
      attribCol); // Meaning no current vao is bound to the opengl context.

  // glVertexAttribPointer(attribPos, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
  // (void*)0); glVertexAttribPointer(attribCol, 3, GL_FLOAT, GL_FALSE, 6 *
  // sizeof(float), (void*)( 3*sizeof(float) ));
  //   replaced with:
  glVertexArrayAttribFormat(
      hctVAO, attribPos, 3, GL_FLOAT, false,
      0); // Need to precise vao, as there is no context binding in DSA
  glVertexArrayAttribFormat(
      hctVAO, attribCol, 3, GL_FLOAT, false,
      3 * sizeof(
              float)); // https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glVertexAttribFormat.xhtml
  ...
  //  ---------------------------
  GLuint shader_prog = create_programme_from_files("shader_triangle.vert",
                                                   "shader_triangle.frag");
