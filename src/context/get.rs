...
impl From<bool> for OxideGLItem {
    fn from(value: bool) -> Self {
        Self::Bool(value)
    }
}
impl From<i32> for OxideGLItem {
    fn from(value: i32) -> Self {
        Self::Int(value)
    }
}
impl From<f32> for OxideGLItem {
    fn from(value: f32) -> Self {
        Self::Float(value)
    }
}
impl From<f64> for OxideGLItem {
...
impl OxideGLContext {
    /*
    fn get(pname: GLenum, idx: Option<GLint>) -> OxideGLItem {
        match pname
    {
        0x0B11 => self.state.point_size.into(), // GL_POINT_SIZE
        0x0B12 => self.state.point_size_range.into(), // GL_POINT_SIZE_RANGE
        0x0B13 => self.state.point_size_granularity.into(), // GL_POINT_SIZE_GRANULARITY
        0x0B21 => self.state.line_width.into(), // GL_LINE_WIDTH
        0x0B22 => self.state.line_width_range.into(), // GL_LINE_WIDTH_RANGE
        0x0B23 => self.state.line_width_granularity.into(), // GL_LINE_WIDTH_GRANULARITY
        ...
        0x821E => self.state.context_flags.into(), // GL_CONTEXT_FLAGS
        ...
        0x9126 => self.state.context_profile_mask.into(), // GL_CONTEXT_PROFILE_MASK
    ...
    pub(crate) fn oxidegl_get_booleanv(&mut self, pname: GLenum, data: *mut GLboolean) {
        panic!("command oxidegl_get_booleanv not yet implemented");
    }
    pub(crate) fn oxidegl_get_doublev(&mut self, pname: GLenum, data: *mut GLdouble) {
        panic!("command oxidegl_get_doublev not yet implemented");
    }
    pub(crate) fn oxidegl_get_floatv(&mut self, pname: GLenum, data: *mut GLfloat) {
        panic!("command oxidegl_get_floatv not yet implemented");
    }
    pub(crate) fn oxidegl_get_integerv(&mut self, pname: GLenum, data: *mut GLint) {
        panic!("command oxidegl_get_integerv not yet implemented");
    }
    pub const VENDOR: &'static [u8] = b"Charles Liske\0";
    pub const RENDERER: &'static [u8] = b"OxideGL\0";
    pub const VERSION: &'static [u8] = b"4.6.0\0";

    fn get_string(name: GLenum) -> *const GLubyte {
        // We don't want the CStr(ing) machinery getting in the way of lifetimes/ownership.
        // C expects const char* (that is valid for 'static), so we give it that
        match name {
            GL_VENDOR => Self::VENDOR.as_ptr(),
            GL_RENDERER => Self::RENDERER.as_ptr(),
            GL_VERSION => Self::VERSION.as_ptr(),
            GL_SHADING_LANGUAGE_VERSION => Self::VERSION.as_ptr(),
            GL_EXTENSIONS => std::ptr::null(),
            _ => {
                panic!("unrecognized Enum")
            }
        }
    }
