...
impl Context {
    fn get<T: GlDstType>(&self, parameter_name: GLenum, ptr: *mut T) {
        unsafe { 0u32.write_all(ptr) } // works
        unsafe { [0u32, 1u32].write_all(ptr) } // also works
        unsafe { true.write_all(ptr) } // also works
    }

    // fn get(&self, parameter_name: GLenum, idx: Option<GLint>) -> OxideGLItem {
    //     let item: PhantomData<*const bool> = match parameter_name {
